...
compatibility: Requires the gds-agent MCP server (PyPI package gds-agent) and neo4j-cypher MCP server (PyPI package neo4j-cypher) connected
...
## Best Practices
1. **Large graphs.** When the graph in the DB large, you might want to consider projecting subgraphs at the start for analysis.
...
2. **Long running tools.** Certain algorithms (or Cypher queries) are long running. For exploratory work, consider try them out on smaller projected graphs before executing them on a desirable large projected raph.
...
4. **Perform additional analysis when needed.** You do not need to use soley the Cypher and GDS tools. 
