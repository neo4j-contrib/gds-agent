...
The extension bundles the MCP server, the `neo4j-graph-analysis` skill, and a context file in one install.
...
4. Verify: `gemini skills list --all` shows `neo4j-graph-analysis`, and `/mcp` inside a session shows the `neo4j-gds` server.
